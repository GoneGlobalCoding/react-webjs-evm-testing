...
// Define a contract
contract AdamCoins is StandardToken {

    // Set Message
    string initMessage = 'ADC - Adam Tokens';

    // Set a name for the token
    string public name = "AdamCoins";
    ...
    // Events
    event TokenInfo(address sender, string symbol, uint supply, string name, string uiInfo);



    function TokenMessage() public view returns (string) {
        return initMessage;
    }

    function MessageSenderAddress() public view returns (address) {
        return msg.sender;
    }

    function InitSupply() public view returns (uint) {
    ...
    function InvokeEvent(string uiInfo) public {
        TokenInfo(msg.sender, symbol, INITIAL_SUPPLY, name, uiInfo);
    }
